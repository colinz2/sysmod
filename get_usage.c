...
static int 
netlink_send_get_data(int socketfd, void *data, int send_len, int recv_len)
{
	int ret;  
	int iSpeepCount = 0;
...
static int 
get_cputm_from_kernel(int offset, cpu_usagetime_t *curr)
{
	int iRet;
	int iSocketFd = 0;
	...
	iRet = netlink_send_get_data(iSocketFd, &stPRData, sizeof(stPRData), sizeof(stPRData));
	if (iRet == 0) {
		if (stPRData.ret != 0) {
			close(iSocketFd);
			return -1;
...
static void 
cal_cpu_usage_result(cpu_usagetime_t *b, cpu_usagetime_t *a, char *buf, int delay) 
{
	...
	sprintf(buf, "%5.2f%% in last %3ds (total:%-5llu idle:%-5llu load:%-5llu io:%-5llu system:%-4llu user:%-5llu irq:%-5llu guest:%-4llu)\n",
