...
static inline uint64_t cputime_get_total(stat_cputime_t *time) {
  return time->user + time->nice + time->system + time->idle + time->iowait +
         time->irq + time->softirq + time->steal + time->guest +
         time->guest_nice;
}

static inline uint64_t cputime_get_load(stat_cputime_t *time) {
  return time->user + time->nice + time->system + time->irq + time->softirq +
         time->steal + time->guest + time->guest_nice;
}
static inline uint64_t cputime_get_idle(stat_cputime_t *time) {
  return time->idle + time->iowait;
}

static inline uint64_t cputime_get_iowait(stat_cputime_t *time) {
  return time->iowait;
}

static inline uint64_t cputime_get_system(stat_cputime_t *time) {
  return time->system;
}

static inline uint64_t cputime_get_user(stat_cputime_t *time) {
  return time->user + time->nice;
}

static inline uint64_t cputime_get_irq(stat_cputime_t *time) {
  return time->irq + time->softirq;
}

static inline uint64_t cputime_get_guest(stat_cputime_t *time) {
  return time->guest + time->guest_nice;
}

struct cputime_rb *cputime_rb_create(uint32_t size) {
...
void cputime_rb_free(struct cputime_rb *rb) {
  if (rb->data != NULL) {
    kfree(rb->data);
    kfree(rb);
  }
...
int cputime_rb_isfull(struct cputime_rb *rb) { return rb->maxsize == rb->size; }

void cputime_rb_push(struct cputime_rb *rb, stat_cputime_t *v) {
...
int cputime_rb_read(struct cputime_rb *rb, int offset, stat_cputime_t *v) {
  int where = 0;
  if (rb->size <= offset) {
    return -1;
  }
...
static uint32_t read_cpu_stat(stat_cputime_t *cpu) {
  struct file *fp;
  loff_t pos = 0;
  char line[256] = {0};
...
static void cal_cpu_usage_result(stat_cputime_t *b, stat_cputime_t *a,
                                 char *buf, int delay) {
  // uint64_t usage;
  ...
  // sprintf(buf, "%-4lu)\n", guest);
  sprintf(buf,
          "in last %3ds (total:%-6lu idle:%-6lu io:%-5lu system:%-4lu "
          "user:%-5lu irq:%-5lu guest:%-4lu)\n",
          delay, total, idle, iowait, system, user, irq, guest);
}

static uint32_t cpu_usge_analyse(struct cputime_rb *rb, stat_cputime_t *cur) {
  stat_cputime_t bf;
  char buf[256] = {0};
  struct file *fp;
  loff_t pos = 0;
...
static int _cpu_usage_sendmsg2user(struct sock *ntsk, int pid,
                                   stat_cputime_t *cpu_info, int ret) {
  int err;
  struct sk_buff *skb2;
  ...
  nlhnew = nlmsg_put(skb2, 0, 0, SYS_USAGE_NTLK_CPU1,
                     sizeof(SYS_USAGE_NETLINK_DATA_S), 0);
  if (nlhnew == NULL) {
    printk(KERN_WARNING "_cpu_usage_sendmsg2user \r\n");
    return -1;
  }
